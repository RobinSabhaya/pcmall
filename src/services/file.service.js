...
const sharpCompress = async (fromFile, toFile, ext, quality, buffer) => {
  try {
    const compress = await sharp(buffer)
      ...
      .then((data) => data)
      .catch(() => false);
    await s3Upload(toFile, compress, { filePathForMimeType: fromFile });
    ...
    throw new ApiError(
      ...
      "",
      "",
      ACTIVITY_LOG_TYPES.error_types.failed_sharp_compress
    );
  }
};
...
exports.compressFileUsingBuffer = async (filePayload, fileQuality) => {
  try {
    ...
    const fileCompress = async (fromFile, fileName, mimeType) => {
      ...
      await s3Upload(`${writeFilePath}/${file_name}`, compress, { mimeType });
    ...
    if (Array.isArray(filePayload.file)) {
      ...
      const fileName = `file_${Date.now()}${ext}`;
      await s3Upload(`${writeFilePath}/${fileName}`, filePayload.file.buffer, {
...
const compressFile = async (fromFile, qualities, toFile, fileName, buffer) => {
  try {
    ...
    throw new ApiError(
      ...
      "",
      "",
      ACTIVITY_LOG_TYPES.error_types.failed_compress_file
    );
  }
};
  ...
  try {
    for (let j = 0; j < filesDtl.length; j++) {
      ...
      if (filesDtl[j].fileUploadType === "single") {
        ...
        await s3Upload(`${writeFilePath}/${fileName}`, buffer, {
    ...
    throw new ApiError(
      ...
      "",
      "",
      ACTIVITY_LOG_TYPES.error_types.failed_to_save_file
    );
  }
};
        ...
        for (const filename of filesObj[key]) {
          ...
          if (!allowedExtensions.includes(extension)) {
            throw new ApiError(
              ...
              "",
              "",
              ACTIVITY_LOG_TYPES.error_types.image_ext_not_valid
            );
          }
        }
        ...
        for (const filename of filesObj[key]) {
          ...
          if (!allowedExtensions.includes(extension)) {
            throw new ApiError(
              ...
              "",
              "",
              ACTIVITY_LOG_TYPES.error_types.doc_ext_not_valid
            );
          }
        }
      } else if (typeof filesObj[key] === "string") {
...
exports.uploadWithoutCompress = async (file, uploadFolder, fileFor) => {
  ...
  switch (fileFor) {
    case "image":
      if (
        ![".png", ".jpg", ".jpeg", ".webp", ".gif", ".tiff"].includes(
          ext.toLowerCase()
        )
      )
        throw new ApiError(
    ...
    case "sound":
      if (![".mp3", ".wav", ".wav", ".ogg", ".m4a"].includes(ext.toLowerCase()))
        throw new ApiError(
          httpStatus.BAD_REQUEST,
          "Only .mp3, .wav, .ogg, .m4a formats are allowed.",
          "",
          "",
          ACTIVITY_LOG_TYPES.error_types.audio_ext_not_valid
        );
      break;
    case "csv":
      if (ext !== ".csv") {
        throw new ApiError(
          ...
          "",
          "",
          ACTIVITY_LOG_TYPES.error_types.csv_ext_not_valid
        );
      }
      break;
    case "document":
      if (![".pdf", ".docx"].includes(ext.toLowerCase())) {
        throw new ApiError(
          httpStatus.BAD_REQUEST,
          "Only .pdf .docx format is allowed.",
          "",
          "",
          ACTIVITY_LOG_TYPES.error_types.document_ext_not_valid
        );
      }
      break;
